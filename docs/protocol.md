...
| `src/events.ts` | Durable event union (`scene.started`, `turn.started`, `turn.committed`) — rows of the append-only event log, replayed via SSE `Last-Event-ID`. strictObject: own formats reject unknown keys (B5). |
| `src/stream.ts` | Ephemeral SSE frames (`hello`, `stream` sentence) — display-only, never durable, never carry an SSE `id:` (B6). |
| `src/commands.ts` | POST command bodies + responses (`start-turn`), user text capped at 8 KB (B7). |
...
- SSE frames: `event: event` + `id: <event-log seq>` + `data: <WeltariEvent JSON>` for durable events; `event: stream` / `event: hello` with no `id:` for ephemeral frames.
