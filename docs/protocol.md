...
| `src/events.ts` | Durable event union (`scene.started`, `scene.ended`, `turn.started`, `turn.committed`, `reflection.committed`, `world_agent.committed`, `job.failed`, `job.parked`) — rows of the append-only event log, replayed via SSE `Last-Event-ID`. strictObject: own formats reject unknown keys (B5). |
...
| `src/commands.ts` | POST command bodies + responses (`start-turn`, `end-scene`, `open-scene`), user text capped at 8 KB (B7). |
