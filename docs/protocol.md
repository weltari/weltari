...
| `src/events.ts` | Durable event union (`scene.started`, `turn.started`, `turn.committed`, `job.failed`, `job.parked`) — rows of the append-only event log, replayed via SSE `Last-Event-ID`. strictObject: own formats reject unknown keys (B5). |
