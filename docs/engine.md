...
| `sublocations.ts` | The sublocation registry (M4 part 2): known = fixture trio ∪ `sublocation.materialized` events per world — ONE projection read by the change_sublocation gate, open-scene's sublocation gate and explore's occupancy gate. Square math for the 8×8 fog grid (`squareOf`/`squareCenter`, protocol `MAP_FOG_GRID`); ids are deterministic per square (`subloc:sq-<col>-<row>`) so retries can never mint twins. |
