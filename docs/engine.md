...
| `sublocations.ts` | The sublocation registry (M4 part 2): known = fixture trio ∪ `sublocation.materialized` ∪ `sublocation.created` (M5 part 2 Flow A — those carry a `footprint` polygon) events per world — ONE projection read by the change_sublocation gate, open-scene's sublocation gate and explore's occupancy gate. Square math for the 8×8 fog grid (`squareOf`/`squareCenter`, protocol `MAP_FOG_GRID`); ids are deterministic per square (`subloc:sq-<col>-<row>`) so retries can never mint twins. Square occupancy counts only fixture/materialized rows: Flow-A sublocations are sub-square features and never block an Explore. |
...
| `fault-points.ts` | The `FaultPoint` union — the kill-harness contract (I4): Week-1 three + M2's `mid_reflection`/`mid_painter`/`mid_cron` + M3's `mid_update` + M4's `mid_materialize` + M5 part 2's `mid_map_edit`. |
