...
**Weltari is an agentic roleplay / gameplay engine.** The user joins a persistent, stateful **world** and roams it freely — closer to an **open-world life-sim (think Stardew Valley)** than a galgame. The world keeps living as fictional time passes: characters move between sublocations (CRON-driven in V1), post to a social feed, and reach out on their own — all fired by CRON replay when the user's own play advances the world clock. **Fictional time never advances while the user is away** (owner ruling 2026-07-10, [§16](#16-cross-cutting-concerns)): the story waits, and resumes exactly where it paused. It is conceptually similar to SillyTavern, but with three defining differences:
...
| **CRON scheduler**    | Fires due **game-time** occurrences into the ledger **whenever the world clock advances** — never on wall clock (owner ruling 2026-07-10): social posts, proactive DM drops, character movement, marker TTL sweeps. World-inert maintenance (compaction) may run on real time.                                                                                                                                                                                                                                                                          |
...
- **Idle = paused, never a timer (owner ruling 2026-07-11):** when the user stops interacting with the scene without closing the app, the scene simply stays paused — indefinitely. There is no idle timeout; nothing closes a scene on wall-clock time. The scene ends **lazily**, at the moment the user's own next action requires it (starting another scene or a character-fired `startscene` runs the standard end-before-open transition with full fan-out), or when the user ends it explicitly. Returning to a paused scene is a true mid-sentence resume. While a scene is paused its characters remain `in_scene` (offline in chat, skipped by CRON picks) — a UI affordance ("return or end?") is the remedy if that ever frustrates, never a timer.
...
CRON drops proactive DMs on a **game-time** schedule (random pick in V1; FEL-driven in V2). **Fires only when the world clock advances** (owner ruling 2026-07-10 — never a wall-clock scheduler): the text is generated for a fictional moment inside the span the user's own play just caused to pass, stamped with that fictional send time, and lands as an unread message — so a character only ever "spends time" inside time that actually passed, and a paused world sends nothing. Re-ask backoff and the 3-unanswered freeze run on the same game-time axis. Enabled by default. **Rev 2 mechanics:**
...
- Daily CRON picks random characters to post about something they experienced — default **2 posts per game day**, user-tunable in config ([§15](#15-config); owner ruling 2026-07-11). **Hard ceiling: 10 posts per time skip** — the freshest window survives, older skipped posts are simply never generated (no player scrolls endless backlog; V1 builds texture and triggers, not an emergent-feed sandbox).
...
- User connects a Telegram/WeChat bot and subscribes to the GM or specific characters (DM pushes only; **no group-chat pushes**). Note that user can subscribe to GM, which will redirect other characters message with "Elias has sent you a message on Weltari! -- Elias:" note that user should be able to custom GM response. Also note when character submit event, the message "character is waiting you at `sublocation`" got pushed. and that invitation carries its character-chosen **game-time entry window** ([§7](#7-character-system--c-module)) — with **no map anchor in V1** (the invitation lives in chat/gateway only; on send the character is reserved via presence, released by a hardcoded routine on expiry). Character still run reflection after they send a scene request.
- **Onboarding is soft (owner ruling 2026-07-10):** gateway setup is never part of cold boot. The GM introduces it later as a normal **unread chat message** in Weltari Chat, and all gateway configuration is reachable from that GM conversation — the GM is the natural push channel for anything setting- or game-relevant. Proactive DMs themselves are on by default with or without a connected messenger ([§8](#8-messaging-system-chat)).
...
| Social post + reactions | Social media.                                                                                                                                                                                                              | 2 posts / game day (default; config)               | LLM                         | Background jobs after landing; counts against skip budget; feed ceiling 10/skip                             |
...
**Retry ceiling + void failsafe (owner ruling 2026-07-10).** Critical chains — the `end_scene` fan-out (reflections, World Agent commit), `reflect_chat`, a character's `startscene` fire — retry up to **10 attempts**, with hardcoded error feedback prompting the model to resubmit when the failure is a malformed tool call. If a step still fails after 10, the whole unit **rolls back as if it never happened**: expressed as a compensating `*.voided` event (the log stays append-only — nothing is ever deleted; projections drop the voided unit), presence released, and the frontend shows a small red line naming the error. Normal operation never reaches this; it is the safe exit.
...
- **No passive advance (owner ruling 2026-07-10 — supersedes the earlier "passive advance" decision):** the clock **never** moves on real time. Only events the user is present for advance it: manual skips, scene-end acceleration (the continue buttons), entering a marker/scene scheduled at a later game time. Storytelling and a self-evolving world pull in opposite directions — the user must be present to be told the story; while they are away the world holds still and resumes exactly where it paused. CRON is the *replay* machinery that fires everything due whenever the clock does advance ([§14](#14-map-system-and-cron)) — never a wall-clock scheduler.
...
One scheduler, one ledger — all **game-time occurrences, fired only when the world clock advances** (owner ruling 2026-07-10; only world-inert maintenance like compaction and profile analysis may run on real time): social posts (default 2/game day, config), proactive DM drops (lazy; eager for gateway subscribers), world movement (mailbox-routed, presence-checked), marker TTL sweeps.
