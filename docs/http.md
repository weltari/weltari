...
| `server.ts` | Fastify 5 instance; `fastify-type-provider-zod` validator+serializer set once (B9); `GET /v1/events`; `POST /v1/commands/start-turn` → injected `startTurn` seam (202 / 409); `POST /v1/commands/end-scene` (202 + jobs_enqueued / 409) and `POST /v1/commands/open-scene` (202 / 409 `blocked_on_pending_jobs`) → scene-lifecycle seams; `POST /v1/commands/advance-time` (202 + new world time + enqueue counts / 409) → WorldClock seam. |
