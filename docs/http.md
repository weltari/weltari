...
| --- | --- |
| `bus.ts` | Generic in-process `Bus<T>`; `EventBus` (durable) + `StreamBus` (ephemeral). Listener throws are contained per-socket. |
| `sse.ts` | `attachSseClient`: hello frame → subscribe-then-replay with a shared cursor (exactly-once, no gap: replay is synchronous) → live tail + heartbeat comments. `Last-Event-ID` header or `?last_event_id=` query. |
