...
| `src/components/NavRail.tsx` | The Left Nav Rail (wireframes §0.1): logo, Scene ▶, Map, Feed, Chats, Wiki, Config; blinking clock (→ Gameday flow) + profile avatar bottom-anchored. Feed stays disabled with a "later" tooltip — never fake surfaces (Wiki went live in M6 part 3). Mobile: becomes a bottom bar (recorded deviation below). |
