...
| --- | --- |
| `types.ts` | The seam: `LlmClient`, `LlmCall`, `LlmUsage`. Everything outside this dir imports only this. `CallKind` covers the scripted turn (`narrator`/`character`/`narration`) plus the cold-path kinds `reflection`, `world_agent`, `materialize` (M4 part 2: the sublocation-stub generation) and `map_edit` (M5 part 2: the Flow-A GM form). M3: `LlmCall.toolset` ('narrator') offers tools; `LlmCallResult.toolCalls` returns RAW calls — callers must run both B6 gates. |
