...
| --- | --- |
| `storage/repositories/ledger.ts` | Sole SQL site: `enqueue` (UNIQUE-key no-op), transactional `claimNext` (serial-group aware), `markCommitted/markRetry/markParked`, `sweepExpiredLeases`. Rows validated on read; garbage is `CorruptStateError`. |
...
Emits `job.failed`, `job.parked` (actor `system:ledger`). Consumes none.
