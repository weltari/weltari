...
const SQLITE = [
  { name: 'better-sqlite3', message: 'Only apps/server/src/storage may touch SQLite (Brief §2.7). Call a repository instead.' },
];
const AI_SDK = ['ai', '@openrouter/ai-sdk-provider', '@ai-sdk/openai-compatible'].map((name) => ({
  name,
  message: 'Only apps/server/src/llm may import the AI SDK. Call the ModelRegistry / LLM client interface instead.',
}));
const GRAMMY = [
  { name: 'grammy', message: 'Only apps/server/src/gateway/telegram may import grammY (trust-boundary fence, Guide B7).' },
];
const MULTIPART = [
  { name: '@fastify/multipart', message: 'Only apps/server/src/boundary/uploads may handle uploads (Guide B13).' },
];
...
export default defineConfig([
  globalIgnores(['**/dist/**', '**/node_modules/**', '**/coverage/**', 'plugins/**']),
  ...
  /* ---- Base TypeScript block: all .ts/.tsx ---- */
  {
    ...
    rules: {
      ...
      'no-restricted-properties': ['error', {
        object: 'process',
        property: 'exit',
        message: 'Only apps/server/src/observability/fatal.ts may exit. Throw a typed AppError instead (Guide C5).',
      }],
      ...
      'no-restricted-syntax': ['error', {
        ...
        message: 'Use .safeParse() via validateAt() — .parse() throws (Guide B1). Non-Zod false positives take a justified inline disable.',
      }],
      /* Misc hard checks */
  ...
  /* ---- A16: no wall-clock reads in the engine (injected clocks only) ---- */
  {
    ...
    rules: {
      'no-restricted-syntax': ['error',
        {
          selector: "CallExpression[callee.property.name='parse'][callee.object.name!='JSON']",
          message: 'Use .safeParse() via validateAt() — .parse() throws (Guide B1).',
        },
        {
          selector: "NewExpression[callee.name='Date']",
          message: 'Engine time is injected: use WorldClock / SystemClock (Guide A16).',
        },
        {
      ...
      ...restricted(ALL_FENCES, [
        {
          ...
          message: 'The frontend consumes @weltari/protocol only — no private side-channels (Brief §1).',
        },
      ]),
    },
  },
      ...
      ...restricted(ALL_FENCES, [
        {
          ...
          message: 'MIT packages must not depend on the AGPL core (license fence, Guide A12).',
        },
      ]),
    },
  },
  ...
  /* ---- Tests: no focus/skip, no snapshots outside protocol, raw SQLite allowed for invariant tests ---- */
  {
    ...
    rules: {
      ...
      'no-restricted-syntax': ['error',
        {
          selector: "CallExpression[callee.property.name='parse'][callee.object.name!='JSON']",
          message: 'Tests use safeParse too — assertions are not exempt (Guide §0.12).',
        },
        {
          selector: "CallExpression[callee.property.name='toMatchSnapshot']",
          message: 'Snapshot tests are sanctioned only in packages/protocol (Guide E5).',
        },
      ],
