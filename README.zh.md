...
它默认**自托管、单进程**:没有订阅、没有哪家公司拥有你的世界。你把它接到一个 AI 提供商,其余一切都留在你自己的硬件上。未来可能会以开源核心(open core)的方式推出托管选项 —— 用于多人联机,也可以只是为了方便那些不想自己搭服务器的人 —— 但核心引擎将始终可以独立运行在你自己掌控的硬件上。并且它从设计上就让一切有意义的事情都被**永久记录** —— 世界会记得。
...
| **M3 —— 玩家体验** | 真正的视觉小说场景页、场景引擎工具、插件加载器、打包 + 签名自更新 | ✅ 已完成 |
| **M4 —— UI 外壳** | 应用外壳、地图/游戏日/设置页面、迷雾与探索、场景生命周期 | ✅ 已完成 |
| **M5 —— 手绘世界地图** | 真实图像后端绘制并扩展地图;可在图上作画、点击进入 | ✅ 已完成 |
| **M6 —— 创造与聊天** | 场景内创造循环、与角色私聊、聊天→场景桥接、主动的世界 | 🚧 进行中 |

目前还没有"一条命令即可游玩"的构建 —— 这份 README 用来介绍项目,而不是提供安装包。想深入了解(英文):[docs/project-overview.md](docs/project-overview.md)(应用是什么、进展到哪、如何试玩)和 [docs/code-tour/](docs/code-tour/README.md)(逐模块的源码导览)。
